...
final_erradicacion = fecha_actual + relativedelta(months=meses['meses_faltantes_esfuerzo_actual'])
...
The table \ref{table:carryingCapacity} shows the number of months to end eradication considering 3 different load capacities and an effort equal to that of the last months (\py{'%.0f'% meses['esfuerzo_actual']} night-traps).
...
\begin{itemize}
\item To complete eradication by January 2020, an effort of \py{'%.0f'% meses['esfuerzo_2019']} night-traps each month must be applied. That means \py{'%.0f'% meses['tramperos_2019']} trappers (with 30 traps each) need to be deployed
\item  TARGET: To complete eradication by July 2020, an effort of \py{'%.0f'% meses['esfuerzo_primer_semestre']} night-traps each month must be applied. That means \py{'%.0f'% meses['tramperos_primer_semestre']} trappers (with 30 traps each) need to be deployed.
\item If current effort from the last expeditions is maintained (\py{'%.0f'% meses['esfuerzo_actual']} night-traps), we will complete eradication in \py{final_erradicacion.strftime("%B %Y")}.
\end{itemize}	
