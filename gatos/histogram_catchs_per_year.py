...
def generate_histogram(**argumentos):
    nombre_archivo_entrada = argumentos["resource"]
    datos_gatos_socorro = pd.read_csv(nombre_archivo_entrada)
    years_in_data = years_from_data(datos_gatos_socorro)
    bar_positions = get_bar_positions(years_in_data)

    esfuerzo_acumulado_anual, gatos_erradicados = calculate_yearly_cumulative_effort_and_captures(
        datos_gatos_socorro, years_in_data
    )
    ...
    ax2.tick_params(labelsize=TAMANO_FUENTE)
    ax2.spines["top"].set_visible(False)
    nombre_imagen_salida = argumentos["output_file"]
    plt.savefig(f"{nombre_imagen_salida}", dpi=300, transparent=True)
    return fig
